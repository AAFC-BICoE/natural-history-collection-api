...
public class ExpeditionCRUDIT extends CollectionModuleBaseIT {
  ...
  private static final String EXPECTED_GEOGRAPHIC_CONTEXT = "Atlantic coast of South America";
  private static final LocalDate EXPECTED_START_DATE = LocalDate.of(1951, 01, 01);
  private static final LocalDate EXPECTED_END_DATE = LocalDate.of(1952, 01, 01);
  private final List<UUID> EXPECT_PARTICIPANTS = List.of(UUID.randomUUID(), UUID.randomUUID());
  ...
  void find() {
    ...
    Assertions.assertEquals(EXPECT_PARTICIPANTS, result.getParticipants());
  ...
  private Expedition buildExpectedExpedition() {
    return ExpeditionFactory.newExpedition()
      ...
      .participants(EXPECT_PARTICIPANTS)
      .build();
  }
