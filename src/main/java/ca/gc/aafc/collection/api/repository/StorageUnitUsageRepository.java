...
public class StorageUnitUsageRepository extends DinaRepositoryV2<StorageUnitUsageDto, StorageUnitUsage> {
  ...
  @Override
  protected Link generateLinkToResource(StorageUnitUsageDto dto) {
    try {
      return linkTo(methodOn(StorageUnitUsageRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @GetMapping(StorageUnitUsageDto.TYPENAME + "/{id}")
  @Transactional(readOnly = true)
  ...
  @PostMapping(path = StorageUnitUsageDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(StorageUnitUsageDto.TYPENAME)
  @Transactional
  ...
  @DeleteMapping(path = StorageUnitUsageDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(StorageUnitUsageDto.TYPENAME + "/{id}")
  @Transactional
