...
public interface CollectionHierarchicalDataDAO {
  ...
  @Select(
    "WITH RECURSIVE get_hierarchy (id, parent_id, uuid, name, rank, target_organism_determination) AS (" +
            "  SELECT initial_t.id, initial_t.parent_material_sample_id, initial_t.uuid, initial_t.material_sample_name, 1, organism.determination" +
            ...
            "  UNION ALL" +
            "    SELECT node.id, node.parent_material_sample_id, node.uuid, node.material_sample_name, gh.rank + 1, organism.determination FROM " +
            ...
            "SELECT id, uuid, name, rank, target_organism_determination FROM get_hierarchy;")
