...
import ca.gc.aafc.collection.api.mapper.AssociationMapper;
...
public class AssociationRepository extends DinaRepositoryV2<AssociationDto, Association> {

  private final DinaAuthenticatedUser dinaAuthenticatedUser;

  public AssociationRepository(
    @NonNull AssociationService dinaService,
    ...
    super(
      ...
      AssociationMapper.INSTANCE,
      AssociationDto.class,
      Association.class,
      buildProperties, objectMapper, new DinaMappingRegistry(AssociationDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }

  @Override
  protected Link generateLinkToResource(AssociationDto dto) {
    try {
      return linkTo(methodOn(AssociationRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @PostMapping(path = AssociationDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(AssociationDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
    throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(AssociationDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = AssociationDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(AssociationDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(AssociationDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = AssociationDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = AssociationDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(AssociationDto.TYPENAME + "/{id}")
  @Transactional
